...
def protein_init(seqs):
    ...
    参数:
        seqs: 蛋白质序列列表
    返回:
        result_dict: 包含蛋白质特征的字典
    """
    result_dict = {}
    model_location = "esm2_t33_650M_UR50D"
    
    # 设置PyTorch Hub的下载目录
    torch.hub.set_dir('F:/Open-source project/MIF-DTI-main/torch_hub_cache')
    ...
    for seq in tqdm(seqs):
        ...
        # 利用ESM模型提取token representation和contact map probability
        ...
        # 根据contact map probability构建图结构
        edge_index, edge_weight = contact_map(contact_map_proba)
        
        result_dict[seq] = {
            'seq':seq,
            'seq_feat':torch.from_numpy(seq_feat), # 物理化学特征
            'token_representation':token_repr.half(), # ESM提取的语义表征
            'num_nodes': len(seq),
            'num_pos':torch.arange(len(seq)).reshape(-1,1),
            'edge_index': edge_index, # 图的边索引
            'edge_weight':  edge_weight, # 图的边权重
        }
